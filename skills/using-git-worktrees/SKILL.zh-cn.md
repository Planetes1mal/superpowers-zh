...
description: 在开始需要与当前工作区隔离的功能工作，或在执行实现计划之前使用 - 创建隔离的 git worktrees，带有智能目录选择和安全验证
---
...
Git worktrees 创建共享相同仓库的隔离工作区，允许同时在多个分支上工作而无需切换。
...
**被调用方：**
- **brainstorming**（第 4 阶段）- 当设计被批准且实施紧随其后时必需
